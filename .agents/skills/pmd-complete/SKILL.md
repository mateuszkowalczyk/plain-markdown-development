...
4. Compare the recorded review baseline with repository state. Confirm that the task path and title; Builder tier; Sources; Depends on; Outcome; Acceptance; Plan; Validation; and the entire reviewed diff across every task-owned file remain unchanged since `PASS`, including configuration, documentation, additions, deletions, and files that were untracked at review time. Confirm no new task-owned changes appeared outside that boundary. Only Coordinator updates to Review, Manual validation, and Status that record completed gates are exempt; any other post-review change makes the review stale. If the baseline cannot establish this, return for a fresh review.
